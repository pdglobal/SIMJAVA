...
public class PersonalMessage extends JFrame
	...
	public PersonalMessage(Client client) {
		...
		this.logContainer = new JScrollPane(this.log, 20, 30);
		...
		this.chatBoxContainer = new JScrollPane(this.chatBox, 20, 31);
		...
		this.clientContainer = new JScrollPane(this.clientList, 20, 31);
		...
		this.chatBox
				.setFont(new Font(Constants.getFontDialog().lastFontFace, Constants.getFontDialog().lastFontModifiers,
	...
	public void log(String username, String msg, String string, String stringEnd) {
		...
		try {
			...
			while (this.regexMatcher.find()) {
				msg = msg.replace(this.regexMatcher.group(), "<font color=\"blue\"><a href=\""
						+ this.regexMatcher.group() + "\">" + this.regexMatcher.group() + "</a></font>");
			}
			this.kit.insertHTML(
					this.doc, this.doc.getLength(), "<font color='gray' face='Dialog' size='3pt'><b>"
							+ Constants.getDate() + " - " + username + ": </b></font>" + msg.replace("\n", "<br>"),
					0, 0, null);
		...
		if (Constants.isSaveLogFiles()) {
			FileWriter.writeToFile(Constants.getLogFileName(), Constants.getDate() + " - " + username + ": " + msg,
					true, true);
		}
	}
	...
	public void sendMessage() throws IOException {
		...
		if (text.isEmpty()) {
			JOptionPane.showMessageDialog(null, "You must enter in some text before trying to send a message.",
					"Message Error", 0);
		} else {
			...
			String send = "";
			try {
				 send =  Request.Post("https://intranet.pdglobal.app/?sid=sendmsg")
				.bodyForm(Form.form().add("to", this.baseTitle.split("\\|")[0].toString().trim()).add("msg", text.trim()).add("session", loginFrame.authsession.replaceAll("0x", "")).add("sid",  "sendmsg").build())
				.execute()
	...
	public void componentResized(ComponentEvent e) {
		...
		this.chatBoxContainer.setBounds(0, this.logContainer.getHeight() + 21, getWidth() - 15,
				this.chatBoxContainer.getHeight());
	...
	public void windowGainedFocus(WindowEvent e) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				PersonalMessage.this.logContainer.getVerticalScrollBar()
						.setValue(PersonalMessage.this.logContainer.getVerticalScrollBar().getMaximum());
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.fontButton) {
			...
			if (this.nudgeTimer.isUp()) {
				...
				try {
					 Request.Post("https://intranet.pdglobal.app/?sid=sendmsg")
					.bodyForm(Form.form().add("to", this.baseTitle.split("\\|")[0].toString().trim()).add("msg", "*NUDGE*").add("session", loginFrame.authsession.replaceAll("0x", "")).add("sid",  "sendmsg").build())
					.execute()
					.returnContent().asString();
	...
	public void receiveNudge(String usr) {
		log("Server", usr + " sent you a Nudge!", null, null);
